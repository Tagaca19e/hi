...
import { generateBG } from "./chateffect.js";
...
export default class ServicesSccreen extends Component {
  ...
  loadMessage(msg, i) {
    if (i == 0) {
      return (
        <p className={"text item-" + i}>
          {msg}
          <span className="animControl" onClick={this.stopAnim.bind(this)}>
            Skip
          </span>
        </p>
      );
                    ...
                    if (el) {
                      el.addEventListener("animationend", (event) => {
                        if (event.animationName === "iwishicould") {
                          if (
                            event.srcElement.className.indexOf("item-9") > -1
                          ) {
                        ...
                        if (
                          event.animationName === "nightslikethis" &&
                          this.state.scroll
                        ) {
